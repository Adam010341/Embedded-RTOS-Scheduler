...
from src.engine.models import TickRecord
...
from src.engine.main_scheduler import run_72hr_simulation
from src.advanced_scheduler import RenewableUncertaintyModel
...
def save_schedule_to_json(trajectory: List[TickRecord], output_path: str):
    ...
    if level2_metrics:
        formatted_data["level2_metrics"] = level2_metrics
        
    with open(output_path, 'w', encoding='utf-8') as f:
        json.dump(formatted_data, f, indent=4, ensure_ascii=False)
    print(f"   [OK] schedule saved: {output_path}")

def build_acceptance_log(sporadic_arr, aperiodic_arr, trajectory):
    log_entries = []
    
    def get_assigned(job_id):
        return [t + 1 for t, record in enumerate(trajectory) if job_id in record.k]
        
    for t_release, tasks in sporadic_arr.items():
        for task in tasks:
            assigned = get_assigned(str(task.id))
            comp = max(assigned) if assigned else None
            abs_d = t_release + task.d - 1
            miss = len(assigned) < task.e or comp is None or comp > abs_d
            log_entries.append({
                "job_id": str(task.id),
                "type": "sporadic",
                ...
                "energy_demand": task.w,
                "assigned_hours": assigned,
                "accepted": len(assigned) > 0,
                "completion_time": comp,
                "miss": miss
            })
            
    for t_release, tasks in aperiodic_arr.items():
        for task in tasks:
            assigned = get_assigned(str(task.id))
            comp = max(assigned) if assigned else None
            abs_d = t_release + task.d - 1 if hasattr(task, 'd') else 72
            miss = len(assigned) < task.e or comp is None or comp > abs_d
            log_entries.append({
                "job_id": str(task.id),
                "type": "aperiodic",
    ...
    log_entries.sort(key=lambda x: (x["release_time"], x["job_id"]))
    return log_entries
...
def main():
    ...
    # 4. 抓取所有情境測資
    demo_file = "input/aperiodic_n_sporadic.json"
    is_demo_mode = os.path.exists(demo_file)
    
    if is_demo_mode:
        scenario_files = [demo_file]
        print(f"\n[*] 偵測到 Demo 專用測資: {demo_file}，切換為單一測試模式...")
    else:
    ...
    for filepath in scenario_files:
        ...
        try:
            ...
            # schedule_result
            if is_demo_mode:
                sched_file = "output/schedule_result.json"
                log_file = "output/acceptance_test_log.json"
            else:
            ...
            log_data = {
                ...
                "level2_enabled": LEVEL2_ENABLED,
                "acceptance_test_log": build_acceptance_log(sporadic_arr, aperiodic_arr, trajectory)
            }
    ...
    if is_demo_mode:
        ...
        args = [
            ...
            "--task-set", "output/task_set.json",
            "--schedule", "output/schedule_result.json",
            "--processor", "input/processor_settings.json",
            "--price", "input/price_72hr.json",
            "--scenario", "input/aperiodic_n_sporadic.json",
            "--output", "output/evaluation_results.json"
        ]
        print(f"\n[*] Running single evaluator: {' '.join(args)}")
